...
#include "MECore/haxe/HaxeModule.h"
#include "HaxeUtils.cpp"

namespace ME::haxe {
    bool HaxeModule::LoadFromMemory(const std::vector<uint8_t>& data) {
        return LoadFromMemory(data.data(), data.size());
    }

    bool HaxeModule::LoadFromMemory(const uint8_t* data, int size) {
        char* msg;
    ...
    HaxeType* HaxeModule::GetType(const TypeName& name) {
    ...
    std::vector<HaxeType*> HaxeModule::GetTypesWithName(const TypeName& name) {
